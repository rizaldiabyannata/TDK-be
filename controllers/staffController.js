...
export const getOrganizationalStructure = async (req, res, next) => {
  try {
    const staffByLevel = await staffService.getOrganizationalStructure();

    // Hitung total staff
    const totalCount = Object.values(staffByLevel).reduce(
      (sum, levelStaff) => sum + levelStaff.length,
      0
    );
